...
int main() {
	...
	//Create a deadlock between Process 1 & Process 2, containing a cycle between R1 & R2
    ...
    for (int i = 0; i < 4; i++) {
        if (graph_add_process(&graph3, 0) != 0) {
            printf("Failed to add process to graph\n");
            return 1;
        }
    }
	...
	//create graph (same graph as shown in the project document under deadlock detection)
