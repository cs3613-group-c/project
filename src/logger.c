...
static pthread_mutex_t time_mutex = PTHREAD_MUTEX_INITIALIZER;  // This lock makes sure only one thing changes the time at a time
...
void increment_sim_time(int units) {
    pthread_mutex_lock(&time_mutex);    // Lock the stopwatch so no one else changes it
...
char* get_formatted_sim_time() {
    ...
    pthread_mutex_lock(&time_mutex);    // Lock time so it doesn't change while we read
...
void log_event(const char* format, ...) {
    ...
    vsnprintf(buffer, sizeof(buffer), format, args);  // Formatting the message with passed arguments
    va_end(args);
