...
bool graph_check_deadlock(resource_alloc_graph_t *graph) {
  ...
  for (int i = 0; i < graph->processes_len; i++) {
    if (!visited[i] && graph_detect_cycle(graph, visited, graph->processes_len,
...
bool graph_detect_cycle(resource_alloc_graph_t *graph,
  ...
  if (recursed[process_id]) {
    return true; // If this branch has an ancestor that is the same node, there
                 // is a cycle. Return true
  }
  ...
  for (int i = 0; i < graph->resources_len; i++) {
    if (graph->processes[process_id].request_list[i] >
        0) // if the process is requesting the resource, check all processes
           // this resource is giving allocated in. These are the connections
           // for our graph.
    {
      for (int j = 0; i < graph->resources_len; j++) {
        if (graph->resources[i].current_allocs[j] >
            0) // if the resource is allocating some of itself to another
               // process, check the process recursively. until a cycle is
               // either found or not
          if (graph_detect_cycle(graph, visited, visited_len, recursed,
                                 recursed_len, process_id)) {
