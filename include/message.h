...
typedef enum {
    // NOTE: This is *not* a message. It is a reserved member that signifies an open slot in our message queue 
    MESSAGE_OPEN_SLOT = 0,
    ...
    // A response from the server that grants the train their requested intersection
    MESSAGE_GRANT,
    // A response from the server that indicates a train should wait for their request intersection
    MESSAGE_WAIT,
...
* In this case, a void* indicates that NULL should be used in place of actual data
 * */
...
#define MSG_OPEN_SLOT (message_t){ MESSAGE_OPEN_SLOT, { .open_slot = NULL } }

/* 
 * Our message queue type. In this case, we're using a dynamic array w/ a pointer to our shared memory space.
 * From there, we have a head index & tail index with a length to track how the queue looks in memory.
 *
 * We *could* utilize a linked list type here but I think an array would suit us just as well and would save us time if we need to search for a node
 *
...
/* Returns the first open index from the head or a negative value if it can't find one */
int queue_find_slot(message_queue_t *queue);

/* Pushes the given message onto the queue. A negative value is returned if an error occurs */
int queue_push(message_queue_t *queue, message_t msg);

/* Pops a message from the given queue */
message_t queue_pop(message_queue_t *queue);
